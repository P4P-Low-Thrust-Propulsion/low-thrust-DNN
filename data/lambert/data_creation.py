...
class TransferGenerator:
    def __init__(self, n, ecc, flag):
        ...
        if self.flag:
            ...
            self.plotter3D = OrbitPlotter3D()
            self.plotter2D = StaticOrbitPlotter(ax=self.ax)
            self.plotter3D.set_attractor(Sun)
            self.plotter2D.set_attractor(Sun)
    ...
    def generate_transfers(self):
        ...
        if self.flag:
            self.plotter3D.plot_ephem(earth_ephem, date_launch, label="Earth at launch position")
            self.plotter2D.plot_body_orbit(Earth, date_launch, label="Earth at launch position", trail=True)
        ...
        for i in tqdm(range(self.num_samples), desc="Generating transfers"):
            while True:
                ...
                # Create orbits from final positions parameters
            ...
            # Append the transfer data along with the lambert velocities to the dataframe
            rel_dist = r_final_rot - r_earth_rot # Calculate relative distance
    ...
    def plot_vectors_3D(r_earth, r_final, v_earth, v_final, v0, vf, i):
        ...
        ax.set_ylabel('Y (km)')
        ax.set_zlabel('Z (km)')
        plt.title('Orbital Transfer Visualisation: Earth to Target (Transfer ' + str(i) + ')')
        if rotated:
            plt.title('Orbital Transfer Visualisation: Earth to Target Rotated (Transfer ' + str(i) + ')')
    ...
    def plot_orbit(self, dest_ephem, date_arrival, ss_earth, man_lambert, i):
        ...
        self.plotter3D.plot_ephem(dest_ephem, date_arrival, label="Final position " + str(i+1), trail=True)
        self.plotter2D.plot_ephem(dest_ephem, date_arrival, label="Final position " + str(i+1), color="Red", trail=True)
        self.plotter3D.plot_maneuver(
            ss_earth,
            man_lambert,
            color="black",
            label="Transfer orbit " + str(i+1),
        )
        self.plotter2D.plot_maneuver(
            ss_earth,
            man_lambert,
            color="black",
            label="Transfer orbit " + str(i+1),
