...
# %%
df = pd.read_csv('../data/processed/transfer_data_mil2.csv')
scaler = MinMaxScaler()
scaler.fit(df)

# %%
# Transform the processed using the fitted scaler and keep it as a DataFrame
...
ax1.plot([min(y_test_array[:n, 0]), max(y_test_array[:n, 0])], [min(y_test_array[:n, 0]), max(y_test_array[:n, 0])],
         color='red', linestyle='--')
...
ax2.plot([min(y_test_array[:n, 1]), max(y_test_array[:n, 1])], [min(y_test_array[:n, 1]), max(y_test_array[:n, 1])],
         color='red', linestyle='--')
...
sigma = 1.96
n = 200
mean_residuals_x = np.mean(errors_array[:n, 0])
std_residuals_x = np.std(errors_array[:n, 0])
mean_residuals_y = np.mean(errors_array[:n, 1])
std_residuals_y = np.std(errors_array[:n, 1])

lower_bound_x = y_pred_array[:n, 0] - sigma * std_residuals_x
upper_bound_x = y_pred_array[:n, 0] + sigma * std_residuals_x
lower_bound_y = y_pred_array[:n, 1] - sigma * std_residuals_y
upper_bound_y = y_pred_array[:n, 1] + sigma * std_residuals_y

fig, (ax1, ax2) = plt.subplots(1, 2, figsize=(12, 6))
ax1.scatter(y_test_array[:n, 0], y_pred_array[:n, 0], alpha=0.5, label='Predictions')
ax1.fill_between(y_test_array[:n, 0], lower_bound_x, upper_bound_x, color='red', alpha=0.2,
...
ax2.fill_between(y_test_array[:n, 1], lower_bound_y, upper_bound_y, color='red', alpha=0.2,
                 label=f'{sigma * 100:.0f}% Prediction Interval')
