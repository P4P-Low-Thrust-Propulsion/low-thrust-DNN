...
from datetime import datetime  # He et al. initialization from https://arxiv.org/abs/1502.01852

he_init = tf.contrib.layers.variance_scaling_initializer()  # This class inherits from Sklearn's BaseEstimator and
# ClassifierMixin
...
class DNNClassifier(BaseEstimator, ClassifierMixin):
    ...
    def _dnn(self, inputs):
        ...
        for layer in range(self.n_hidden_layers):  # Apply dropout if specified
            ...
            inputs = tf.layers.dense(inputs, self.n_neurons,
                                     ...
                                     name="hidden{}".format(layer + 1))  # Apply batch normalization if specified
    ...
    def _construct_graph(self, n_inputs, n_outputs):
        ...
        # Placeholders for training processed, labels are class exclusive integers
        ...
        accuracy = tf.reduce_mean(tf.cast(correct, tf.float32), name="accuracy")  # Initializer and saver
    ...
    def _restore_model_parameters(self, model_params):
        ...
        assign_ops = {gvar_name: self._graph.get_operation_by_name(gvar_name + "/Assign") for gvar_name in
                      gvar_names}  # Fetch the initialization values of the assignment operations
    ...
    def fit(self, X, y, n_epochs=100, X_valid=None, y_valid=None):
        # Method to train the model. Implements early stopping if validation processed is provided
        ...
        with self._session.as_default() as sess:
            ...
            for epoch in range(n_epochs):
                ...
                for rnd_indices in np.array_split(rnd_idx, num_instances // self.batch_size):
                    ...
                    train_acc, _ = sess.run([self._accuracy, self._training_op],
                                            feed_dict)  # Early stopping implementation
                if X_valid is not None and y_valid is not None:
                    feed_dict_valid = {self._X: X_valid, self._y: y_valid}  # Write summary for tensorboard
                    if self.tensorboard_logdir:
                    ...
                    if self.show_progress:
                        if epoch % self.show_progress == 0:
                            print(
                                "Epoch: {} Current training accuracy: {:.4f} Validation Accuracy: {:.4f} Validation Loss {:.6f}".format(
                                    epoch + 1, train_acc, val_acc, val_loss))  # Check to see if model is improving
